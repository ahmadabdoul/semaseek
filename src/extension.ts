/**
  extension.reindex-prompt-v2.ts - semantic-code-search (filewatcher + highlight)
  ...
  - Adds a FileSystemWatcher to keep index updated on file changes (create/change/delete)
  - Clicking a search result opens the file and selects/highlights the exact chunk range
...
function getConfig() {
  ...
  return {
    ...
    fileGlob: conf.get<string>('fileGlob', '**/*.{js,ts,jsx,tsx,py,java,go,rs,md,html,css}')
  };
}
...
class InMemoryVectorStore {
  ...
  removeByUri(uri: string) { this.items = this.items.filter(i => i.uri !== uri); }
  removeById(id: string) { this.items = this.items.filter(i => i.id !== id); }
...
async function deleteChunksByUri(uri: string) {
  if (!usingSqlite || !db) return;
  try {
    await dbRunAsync('DELETE FROM chunks WHERE uri = ?', [uri]);
    output.appendLine(`Deleted chunks for ${uri} from DB`);
  } catch (err) {
    output.appendLine('Failed to delete chunks by uri: ' + getErrorMessage(err));
  }
}
  ...
  if (usingSqlite && db) {
    try {
      ...
      for (const r of rows) {
        ...
        vectorStore.add({ id: r.id, uri: r.uri, start: r.start, end: r.end, text: r.text, embedding });
      }
...
async function clearDB(context: vscode.ExtensionContext) {
  if (usingSqlite && db) {
    try {
      ...
      output.appendLine('Cleared sqlite DB chunks');
    } catch (err) {
...
async function indexWorkspace(progress: vscode.Progress<{ message?: string; increment?: number }>, token: vscode.CancellationToken, context?: vscode.ExtensionContext) {
  ...
  const fileGlob = getConfig().fileGlob;
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const queryCommand = vscode.commands.registerCommand('semanticSearch.query', async () => {
    ...
    const items = results.map(r => ({ label: `${(r.score * 100).toFixed(1)}% — ${path.basename(r.item.uri)}`, description: truncate(r.item.text.replace(/\s+/g, ' '), 200), uri: r.item.uri, start: r.item.start, end: r.item.end, score: r.score }));
    ...
    // optionally create a temporary decoration to visually highlight
    const decoType = vscode.window.createTextEditorDecorationType({ backgroundColor: new vscode.ThemeColor('editor.selectionBackground') });
  ...
  try {
    ...
    watcher.onDidDelete(async uri => {
      ...
      vectorStore.removeByUri(uri.toString());
      if (usingSqlite && db) await deleteChunksByUri(uri.toString());
      else {
        try {
          const filePath = path.join(context.globalStorageUri.fsPath, 'semantic_index.json');
          const raw = await fs.readFile(filePath, 'utf8').catch(() => '[]');
